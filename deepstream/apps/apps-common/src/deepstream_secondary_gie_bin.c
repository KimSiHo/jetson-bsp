...
static void write_infer_output_to_file(GstBuffer *buf, NvDsInferNetworkInfo *network_info,
                                       NvDsInferLayerInfo *layers_info, guint num_layers, guint batch_size,
                                       gpointer user_data) {
	NvDsGieConfig *config = (NvDsGieConfig *)user_data;
	guint i;

	/* "gst_buffer_get_nvstream_meta()" can be called on the GstBuffer to get
	...
	for (i = 0; i < num_layers; i++) {
		...
		g_snprintf(file_name, PATH_MAX, "%s/%s_batch%010lu_batchsize%02d.bin", config->raw_output_directory, layer_name,
 ...
 */
static gboolean create_secondary_gie(NvDsGieConfig *configs1, NvDsSecondaryGieBinSubBin *subbins1, GstBin *bin,
                                     guint index) {
	gboolean ret = FALSE;
	gchar elem_name[50];
	...
	case NV_DS_GIE_PLUGIN_INFER_SERVER:
		subbin->secondary_gie = gst_element_factory_make(NVDS_ELEM_INFER_SERVER, elem_name);
		break;
	default:
		NVGSTDS_ERR_MSG_V("Failed to create %s on unknown plugin_type", elem_name);
		goto done;
	}
	...
	for (i = 0; i < config->num_operate_on_class_ids; i++) {
		g_snprintf(operate_on_class_str + strlen(operate_on_class_str),
		           sizeof(operate_on_class_str) - strlen(operate_on_class_str) - 1,
		           "%d:", config->list_operate_on_class_ids[i]);
	}

	g_object_set(G_OBJECT(subbin->secondary_gie), "config-file-path", GET_FILE_PATH(config->config_file_path),
	             "process-mode", 2, NULL);

	g_object_set(G_OBJECT(subbin->secondary_gie), "input-tensor-meta", config->input_tensor_meta, NULL);

	if (config->num_operate_on_class_ids != 0)
		g_object_set(G_OBJECT(subbin->secondary_gie), "infer-on-class-ids", operate_on_class_str, NULL);

	if (config->is_batch_size_set)
		g_object_set(G_OBJECT(subbin->secondary_gie), "batch-size", config->batch_size, NULL);

	if (config->is_interval_set)
		g_object_set(G_OBJECT(subbin->secondary_gie), "interval", config->interval, NULL);

	if (config->is_unique_id_set)
		g_object_set(G_OBJECT(subbin->secondary_gie), "unique-id", config->unique_id, NULL);

	if (config->is_operate_on_gie_id_set)
		g_object_set(G_OBJECT(subbin->secondary_gie), "infer-on-gie-id", config->operate_on_gie_id, NULL);

	if (config->raw_output_directory) {
		g_object_set(G_OBJECT(subbin->secondary_gie), "raw-output-generated-callback", out_callback,
		             "raw-output-generated-userdata", config, NULL);
	}

	if (config->is_gpu_id_set && NV_DS_GIE_PLUGIN_INFER_SERVER == config->plugin_type) {
		NVGSTDS_WARN_MSG_V("gpu-id: %u in sgie: %s is ignored, only accept "
	...
	if (NV_DS_GIE_PLUGIN_INFER == config->plugin_type) {
		...
		if (config->is_gpu_id_set)
			g_object_set(G_OBJECT(subbin->secondary_gie), "gpu-id", config->gpu_id, NULL);
	}
	...
	if (subbin->num_children == 0) {
		...
		g_object_set(G_OBJECT(subbin->sink), "async", FALSE, "sync", FALSE, "enable-last-sample", FALSE, NULL);
	}
 ...
 */
static gboolean should_create_secondary_gie(NvDsGieConfig *config_array, guint num_configs,
	...
	for (i = 0; i < num_configs; i++) {
		if (config->operate_on_gie_id == (gint)config_array[i].unique_id) {
			if (should_create_secondary_gie(config_array, num_configs, bins, i, primary_gie_id)) {
				bins[index].create = TRUE;
...
gboolean create_secondary_gie_bin(guint num_secondary_gie, guint primary_gie_unique_id, NvDsGieConfig *config_array,
	...
	bin->queue = gst_element_factory_make(NVDS_ELEM_QUEUE, "secondary_gie_bin_queue");
	if (!bin->queue) {
	...
	for (i = 0; i < num_secondary_gie; i++) {
		should_create_secondary_gie(config_array, num_secondary_gie, bin->sub_bins, i, primary_gie_unique_id);
	}

	for (i = 0; i < num_secondary_gie; i++) {
		if (bin->sub_bins[i].create) {
			if (!create_secondary_gie(config_array, bin->sub_bins, GST_BIN(bin->bin), i)) {
				goto done;
			}
		}
	}
		...
		if (bin->sub_bins[i].create) {
			if (bin->sub_bins[i].parent_index == -1) {
				...
				if (bin->sub_bins[bin->sub_bins[i].parent_index].tee) {
					...
					NVGSTDS_LINK_ELEMENT(bin->sub_bins[bin->sub_bins[i].parent_index].secondary_gie,
					                     bin->sub_bins[i].secondary_gie);
				}
			}
		}
	}
