...
gboolean create_osd_bin(NvDsOSDConfig *config, NvDsOSDBin *bin) {
	...
	g_object_set(G_OBJECT(bin->nvosd), "display-clock", config->enable_clock, "clock-font", config->font,
	             "x-clock-offset", config->clock_x_offset, "y-clock-offset", config->clock_y_offset, "clock-color",
	             clk_color, "clock-font-size", config->clock_text_size, "process-mode", config->mode, NULL);

	gst_bin_add_many(GST_BIN(bin->bin), bin->queue, bin->nvvidconv, bin->conv_queue, bin->nvosd, NULL);
