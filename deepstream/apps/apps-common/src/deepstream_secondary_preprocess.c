 ...
 * This is needed because all secondary preprocess process same buffer in
 * parallel.
 */
 ...
 */
static gboolean create_secondary_preprocess(NvDsPreProcessConfig *configs1, NvDsSecondaryPreProcessBinSubBin *subbins1,
                                            GstBin *bin, guint index) {
	gboolean ret = FALSE;
	gchar elem_name[50];
	...
	subbin->secondary_preprocess = gst_element_factory_make(NVDS_ELEM_SECONDARY_PREPROCESS, elem_name);
	...
	g_object_set(G_OBJECT(subbin->secondary_preprocess), "config-file", config->config_file_path, NULL);

	if (config->is_operate_on_gie_id_set) {
		g_object_set(G_OBJECT(subbin->secondary_preprocess), "operate-on-gie-id", config->operate_on_gie_id, NULL);
	}
 ...
 */
static gboolean should_create_secondary_preprocess(NvDsPreProcessConfig *config_array, guint num_configs,
                                                   NvDsSecondaryPreProcessBinSubBin *bins, guint index,
                                                   gint primary_gie_id) {
		...
		if (bin->sub_bins[i].create) {
			if (bin->sub_bins[i].parent_index == -1) {
				...
				if (bin->sub_bins[bin->sub_bins[i].parent_index].tee) {
					...
					NVGSTDS_LINK_ELEMENT(bin->sub_bins[bin->sub_bins[i].parent_index].secondary_preprocess,
