...
gboolean create_dsexample_bin(NvDsDsExampleConfig *config, NvDsDsExampleBin *bin) {
	...
	gst_bin_add_many(GST_BIN(bin->bin), bin->queue, bin->pre_conv, bin->cap_filter, bin->elem_dsexample, NULL);
	...
	g_object_set(G_OBJECT(bin->elem_dsexample), "full-frame", config->full_frame, "processing-width",
	             config->processing_width, "processing-height", config->processing_height, "unique-id",
	             config->unique_id, "gpu-id", config->gpu_id, "blur-objects", config->blur_objects, NULL);
