...
gboolean create_primary_gie_bin(NvDsGieConfig *config, NvDsPrimaryGieBin *bin) {
	...
	bin->nvvidconv = gst_element_factory_make(NVDS_ELEM_VIDEO_CONV, "primary_gie_conv");
	if (!bin->nvvidconv) {
	...
	g_object_set(G_OBJECT(bin->primary_gie), "config-file-path", GET_FILE_PATH(config->config_file_path),
	             "process-mode", 1, NULL);
	...
	if (config->is_interval_set)
		g_object_set(G_OBJECT(bin->primary_gie), "interval", config->interval, NULL);
	...
	if (NV_DS_GIE_PLUGIN_INFER == config->plugin_type) {
		...
		if (config->model_engine_file_path) {
			g_object_set(G_OBJECT(bin->primary_gie), "model-engine-file", GET_FILE_PATH(config->model_engine_file_path),
			             NULL);
		}
	}
