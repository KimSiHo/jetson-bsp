...
gboolean set_streammux_properties(NvDsStreammuxConfig *config, GstElement *element) {
	...
	if (config->config_file_path) {
		g_object_set(G_OBJECT(element), "config-file-path", GET_FILE_PATH(config->config_file_path), NULL);
	}

	g_object_set(G_OBJECT(element), "frame-duration", config->frame_duration, NULL);

	g_object_set(G_OBJECT(element), "frame-num-reset-on-stream-reset", config->frame_num_reset_on_stream_reset, NULL);
	...
	if (config->extract_sei_type5_data) {
		g_object_set(G_OBJECT(element), "extract-sei-type5-data", config->extract_sei_type5_data, NULL);
	}
	if (config->num_surface_per_frame > 1) {
		g_object_set(G_OBJECT(element), "num-surfaces-per-frame", config->num_surface_per_frame, NULL);
	}
