...
static gboolean create_render_bin(NvDsSinkRenderConfig *config, NvDsSinkBinSubBin *bin) {
	...
	case NV_DS_SINK_RENDER_3D:
		...
		g_object_set(G_OBJECT(bin->sink), "window-x", config->offset_x, "window-y", config->offset_y, "window-width",
		             config->width, "window-height", config->height, NULL);
	...
	g_object_set(G_OBJECT(bin->sink), "sync", config->sync, "max-lateness", -1, "async", FALSE, "qos", config->qos,
	             NULL);
...
static gboolean create_msg_conv_broker_bin(NvDsSinkMsgConvBrokerConfig *config, NvDsSinkBinSubBin *bin) {
	...
	if (!config->disable_msgconv)
		g_object_set(G_OBJECT(bin->transform), "config", config->config_file_path, "msg2p-lib",
		             ...
		             config->debug_payload_dir, "multiple-payloads", config->multiple_payloads, "msg2p-newapi",
		             config->conv_msg2p_new_api, "frame-interval", config->conv_frame_interval, "dummy-payload",
		             config->conv_dummy_payload, NULL);
	...
	g_object_set(G_OBJECT(bin->sink), "proto-lib", config->proto_lib, "conn-str", config->conn_str, "topic",
	             config->topic, "sync", config->sync, "async", FALSE, "config", config->broker_config_file_path,
	             "comp-id", config->broker_comp_id, "new-api", config->new_api, "sleep-time", config->broker_sleep_time,
	             NULL);
...
static gboolean create_encode_file_bin(NvDsSinkEncoderConfig *config, NvDsSinkBinSubBin *bin) {
	...
	NVGSTDS_ELEM_ADD_PROBE(probe_id, bin->encoder, "sink", seek_query_drop_prob, GST_PAD_PROBE_TYPE_QUERY_UPSTREAM,
	                       bin);
...
static gboolean start_rtsp_streaming(guint rtsp_port_num, guint updsink_port_num, NvDsEncoderType enctype,
...
static gboolean create_udpsink_bin(NvDsSinkEncoderConfig *config, NvDsSinkBinSubBin *bin) {
	...
	g_object_set(G_OBJECT(bin->sink), "host", "127.0.0.1", "port", config->udp_port, "async", FALSE, "sync",
	             config->sync, NULL);
