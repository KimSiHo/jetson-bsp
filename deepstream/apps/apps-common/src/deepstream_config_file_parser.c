...
gboolean parse_labels_file(NvDsGieConfig *config) {
	...
	label_file = g_io_channel_new_file(GET_FILE_PATH(config->label_file_path), "r", &err);

	if (err) {
		NVGSTDS_ERR_MSG_V("Failed to open label file '%s':%s", config->label_file_path, err->message);
		goto done;
	}
	...
	do {
		...
		status = g_io_channel_read_line(label_file, (gchar **)&line_str, NULL, &term_pos, &err);
		 ...
		 */
		label_outputs_length_list = g_list_prepend(label_outputs_length_list, (gchar *)NULL + label_outputs_length);
...
gboolean parse_source(NvDsSourceConfig *config, GKeyFile *key_file, gchar *group, gchar *cfg_file_path) {
	...
	if (g_strcmp0(group, CONFIG_GROUP_SOURCE_ALL)) {
		if (g_key_file_get_integer(key_file, group, CONFIG_GROUP_ENABLE, &error) == FALSE || error != NULL) {
			return TRUE;
		}
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_TYPE)) {
			config->type = (NvDsSourceType)g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_TYPE, &error);
			CHECK_ERROR(error);
			...
			config->camera_csi_sensor_id =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_CAMERA_CSI_SID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_CAMERA_V4L2_DEVNODE)) {
			config->camera_v4l2_dev_node =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_CAMERA_V4L2_DEVNODE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_ALSA_DEVICE)) {
			config->alsa_device = g_key_file_get_string(key_file, group, CONFIG_GROUP_SOURCE_ALSA_DEVICE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_UDP_BUFFER_SIZE)) {
			config->udp_buffer_size =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_UDP_BUFFER_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_VIDEO_FORMAT)) {
			config->video_format = g_key_file_get_string(key_file, group, CONFIG_GROUP_SOURCE_VIDEO_FORMAT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_URI)) {
			gchar *uri = g_key_file_get_string(key_file, group, CONFIG_GROUP_SOURCE_URI, &error);
			CHECK_ERROR(error);
			...
			config->latency = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_LATENCY, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_NUM_SOURCES)) {
			config->num_sources = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_NUM_SOURCES, &error);
			CHECK_ERROR(error);
			if (config->num_sources < 1) {
				config->num_sources = 1;
			}
		} else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
			config->gpu_id = g_key_file_get_integer(key_file, group, CONFIG_GPU_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_NUM_DECODE_SURFACES)) {
			config->num_decode_surfaces =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_NUM_DECODE_SURFACES, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_NUM_EXTRA_SURFACES)) {
			config->num_extra_surfaces =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_NUM_EXTRA_SURFACES, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_DROP_FRAME_INTERVAL)) {
			config->drop_frame_interval =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_DROP_FRAME_INTERVAL, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_CAMERA_ID)) {
			config->camera_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_CAMERA_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_RTSP_RECONNECT_INTERVAL_SEC)) {
			config->rtsp_reconnect_interval_sec =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_RTSP_RECONNECT_INTERVAL_SEC, &error);
			CHECK_ERROR(error);
			...
			config->select_rtp_protocol =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SELECT_RTP_PROTOCOL, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_ID)) {
			config->source_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SMART_RECORD_ENABLE)) {
			config->smart_record =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_ENABLE, &error);
			CHECK_ERROR(error);
			...
			config->smart_rec_cache_size =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_CACHE_SIZE_LEGACY, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SMART_RECORD_CACHE_SIZE)) {
			config->smart_rec_cache_size =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_CACHE_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SMART_RECORD_CONTAINER)) {
			config->smart_rec_container =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_CONTAINER, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SMART_RECORD_START_TIME)) {
			config->smart_rec_start_time =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_START_TIME, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SMART_RECORD_DEFAULT_DURATION)) {
			config->smart_rec_def_duration =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_DEFAULT_DURATION, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SMART_RECORD_DURATION)) {
			config->smart_rec_duration =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_DURATION, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SMART_RECORD_INTERVAL)) {
			config->smart_rec_interval =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_SMART_RECORD_INTERVAL, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_EXTRACT_SEI_TYPE5_DATA)) {
			config->extract_sei_type5_data = g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE,
			...
			config->nvvideoconvert_copy_hw =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_NVVIDEOCONVERT_COPY_HW, &error);
			CHECK_ERROR(error);
		}
#endif
		else {
...
gboolean parse_streammux(NvDsStreammuxConfig *config, GKeyFile *key_file, gchar *cfg_file_path) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_COMPUTE_HW)) {
			config->compute_hw =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_COMPUTE_HW, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_WIDTH)) {
			config->pipeline_width =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_WIDTH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_HEIGHT)) {
			config->pipeline_height =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_HEIGHT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
			config->gpu_id = g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GPU_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_ENABLE_PADDING)) {
			config->enable_padding =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_ENABLE_PADDING, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_FRAME_DURATION)) {
			config->frame_duration =
			    g_key_file_get_uint64(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_FRAME_DURATION, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_BUFFER_POOL_SIZE)) {
			config->buffer_pool_size = g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX,
			                                                  CONFIG_GROUP_STREAMMUX_BUFFER_POOL_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_BATCH_SIZE)) {
			config->batch_size =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_BATCH_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_LIVE_SOURCE)) {
			config->live_source =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_LIVE_SOURCE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_ATTACH_SYS_TS_AS_NTP)) {
			config->attach_sys_ts_as_ntp = g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX,
			...
			config->frame_num_reset_on_stream_reset = g_key_file_get_boolean(
			    key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_FRAME_NUM_RESET_ON_STREAM_RESET, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_BATCHED_PUSH_TIMEOUT)) {
			config->batched_push_timeout = g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX,
			                                                      CONFIG_GROUP_STREAMMUX_BATCHED_PUSH_TIMEOUT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
			config->nvbuf_memory_type =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_NVBUF_MEMORY_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_CONFIG_FILE_PATH)) {
			config->config_file_path = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, CONFIG_GROUP_STREAMMUX,
			                                         CONFIG_GROUP_STREAMMUX_CONFIG_FILE_PATH, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_SYNC_INPUTS)) {
			config->sync_inputs =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_SYNC_INPUTS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_MAX_LATENCY)) {
			config->max_latency =
			    g_key_file_get_uint64(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_MAX_LATENCY, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_FRAME_NUM_RESET_ON_EOS)) {
			config->frame_num_reset_on_eos = g_key_file_get_boolean(
			    key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_FRAME_NUM_RESET_ON_EOS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_ASYNC_PROCESS)) {
			config->async_process =
			    g_key_file_get_boolean(key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_ASYNC_PROCESS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_DROP_PIPELINE_EOS)) {
			config->no_pipeline_eos = g_key_file_get_boolean(key_file, CONFIG_GROUP_STREAMMUX,
			                                                 CONFIG_GROUP_STREAMMUX_DROP_PIPELINE_EOS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_NUM_SURFACES_PER_FRAME)) {
			config->num_surface_per_frame = g_key_file_get_integer(
			    key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_NUM_SURFACES_PER_FRAME, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_EXTRACT_SEI_TYPE5_DATA)) {
			config->extract_sei_type5_data = g_key_file_get_integer(
			    key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_EXTRACT_SEI_TYPE5_DATA, &error);
			CHECK_ERROR(error);
		} else {
			NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, CONFIG_GROUP_STREAMMUX);
		}
	}
...
gboolean parse_dsexample(NvDsDsExampleConfig *config, GKeyFile *key_file) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			config->enable = g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_ENABLE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_FULL_FRAME)) {
			config->full_frame =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_FULL_FRAME, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_PROCESSING_WIDTH)) {
			config->processing_width = g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE,
			                                                  CONFIG_GROUP_DSEXAMPLE_PROCESSING_WIDTH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_PROCESSING_HEIGHT)) {
			config->processing_height = g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE,
			                                                   CONFIG_GROUP_DSEXAMPLE_PROCESSING_HEIGHT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_BLUR_OBJECTS)) {
			config->blur_objects =
			    g_key_file_get_boolean(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_BLUR_OBJECTS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_UNIQUE_ID)) {
			config->unique_id =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_UNIQUE_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_GPU_ID)) {
			config->gpu_id =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_GPU_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
			config->nvbuf_memory_type =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_NVBUF_MEMORY_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_DSEXAMPLE_BATCH_SIZE)) {
			config->batch_size =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE, CONFIG_GROUP_DSEXAMPLE_BATCH_SIZE, &error);
			CHECK_ERROR(error);
		} else {
			NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, CONFIG_GROUP_DSEXAMPLE);
		}
	}
...
gboolean parse_osd(NvDsOSDConfig *config, GKeyFile *key_file) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			config->enable = g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_ENABLE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_MODE)) {
			config->mode =
			    (NvOSD_Mode)g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_MODE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_BORDER_WIDTH)) {
			config->border_width =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_BORDER_WIDTH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_TEXT_SIZE)) {
			config->text_size = g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_TEXT_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_TEXT_COLOR)) {
			gsize length = 0;
			gdouble *list =
			    g_key_file_get_double_list(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_TEXT_COLOR, &length, &error);
			CHECK_ERROR(error);
			if (length != 4) {
				NVGSTDS_ERR_MSG_V("Color params should be exactly 4 floats {r, g, b, "
				                  "a} between 0 and 1");
				goto done;
			}
			...
			gsize length = 0;
			gdouble *list =
			    g_key_file_get_double_list(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_TEXT_BG_COLOR, &length, &error);
			CHECK_ERROR(error);
			if (length != 4) {
				NVGSTDS_ERR_MSG_V("Color params should be exactly 4 floats {r, g, b, "
				                  "a} between 0 and 1");
			...
			config->font = g_key_file_get_string(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_FONT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_CLOCK_ENABLE)) {
			config->enable_clock =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_CLOCK_ENABLE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_CLOCK_X_OFFSET)) {
			config->clock_x_offset =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_CLOCK_X_OFFSET, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_CLOCK_Y_OFFSET)) {
			config->clock_y_offset =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_CLOCK_Y_OFFSET, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_CLOCK_TEXT_SIZE)) {
			config->clock_text_size =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_CLOCK_TEXT_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_HW_BLEND_COLOR_ATTR)) {
			config->hw_blend_color_attr =
			    g_key_file_get_string(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_HW_BLEND_COLOR_ATTR, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
			config->nvbuf_memory_type =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_NVBUF_MEMORY_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_CLOCK_COLOR)) {
			gsize length = 0;
			gdouble *list =
			    g_key_file_get_double_list(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_CLOCK_COLOR, &length, &error);
			CHECK_ERROR(error);
			if (length != 4) {
				NVGSTDS_ERR_MSG_V("Color params should be exactly 4 floats {r, g, b, "
				                  "a} between 0 and 1");
			...
			config->gpu_id = g_key_file_get_integer(key_file, CONFIG_GROUP_OSD, CONFIG_GPU_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_SHOW_TEXT)) {
			config->draw_text = g_key_file_get_boolean(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_SHOW_TEXT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_SHOW_BBOX)) {
			config->draw_bbox = g_key_file_get_boolean(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_SHOW_BBOX, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_OSD_SHOW_MASK)) {
			config->draw_mask = g_key_file_get_boolean(key_file, CONFIG_GROUP_OSD, CONFIG_GROUP_OSD_SHOW_MASK, &error);
			CHECK_ERROR(error);
		} else {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			...
			config->operate_on_gie_id =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_ID_FOR_OPERATION, &error);
			CHECK_ERROR(error);
			config->is_operate_on_gie_id_set = TRUE;
		} else if (!g_strcmp0(*key, CONFIG_GROUP_PREPROCESS_CONFIG_FILE)) {
			config->config_file_path = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_PREPROCESS_CONFIG_FILE, &error));
			CHECK_ERROR(error);
		} else {
			NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, group);
		}
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			...
			config->input_tensor_meta =
			    g_key_file_get_boolean(key_file, group, CONFIG_GROUP_GIE_INPUT_TENSOR_META, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_CLASS_IDS_FOR_OPERATION)) {
			gsize length = 0;
			config->list_operate_on_class_ids =
			    g_key_file_get_integer_list(key_file, group, CONFIG_GROUP_GIE_CLASS_IDS_FOR_OPERATION, &length, &error);
			config->num_operate_on_class_ids = length;
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_BATCH_SIZE)) {
			config->batch_size = g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_BATCH_SIZE, &error);
			config->is_batch_size_set = TRUE;
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_MODEL_ENGINE)) {
			config->model_engine_file_path = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_GIE_MODEL_ENGINE, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_PLUGIN_TYPE)) {
			config->plugin_type =
			    (NvDsGiePluginType)g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_PLUGIN_TYPE, &error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_AUDIO_TRANSFORM)) {
			config->audio_transform = g_key_file_get_string(key_file, group, CONFIG_GROUP_GIE_AUDIO_TRANSFORM, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_FRAME_SIZE)) {
			config->frame_size = g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_FRAME_SIZE, &error);
			config->is_frame_size_set = TRUE;
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_HOP_SIZE)) {
			config->hop_size = g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_HOP_SIZE, &error);
			config->is_hop_size_set = TRUE;
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_AUDIO_RATE)) {
			config->input_audio_rate = g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_AUDIO_RATE, &error);
			config->is_hop_size_set = TRUE;
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_LABEL)) {
			config->label_file_path = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_GIE_LABEL, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_CONFIG_FILE)) {
			config->config_file_path = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_GIE_CONFIG_FILE, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_INTERVAL)) {
			config->interval = g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_INTERVAL, &error);
			config->is_interval_set = TRUE;
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_GIE_UNIQUE_ID)) {
			config->unique_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_GIE_UNIQUE_ID, &error);
			...
			config->is_operate_on_gie_id_set = TRUE;
			CHECK_ERROR(error);
		} else if (!strncmp(*key, CONFIG_GROUP_GIE_BBOX_BORDER_COLOR, sizeof(CONFIG_GROUP_GIE_BBOX_BORDER_COLOR) - 1)) {
			 ...
			 * classes. For generic key "bbox-border-color", strlen (key1) will
			 * return 0 and· class_index will be -1. For class-specific key
			 * "bbox-border-color<class-id>", strlen (key1) will return a
			 * positive value and·class_index will have a value >= 0.
			 */
			if (strlen(key1) > 0) {
			...
			if (class_index == -1) {
				...
				g_hash_table_insert(config->bbox_border_color_table, class_index + (gchar *)NULL, clr_params);
			}
		...
		} else if (!strncmp(*key, CONFIG_GROUP_GIE_BBOX_BG_COLOR, sizeof(CONFIG_GROUP_GIE_BBOX_BG_COLOR) - 1)) {
			NvOSD_ColorParams *clr_params;
			...
			/* Check if the key is specified for a particular class or for all
			 * classes. For generic key "bbox-bg-color", strlen (key1) will
			 * return 0 and· class_index will be -1. For class-specific key
			 * "bbox-bg-color<class-id>", strlen (key1) will return a positive
			 * value and·class_index will have a value >= 0.
			 */
			if (strlen(key1) > 0) {
				class_index = g_ascii_strtoll(key1, &endptr, 10);
				if (class_index == 0 && endptr == key1) {
					NVGSTDS_WARN_MSG_V("BBOX background colors should be "
					                   "specified with key '%s%%d'",
			...
			if (class_index == -1) {
				...
				clr_params = (NvOSD_ColorParams *)g_malloc(sizeof(NvOSD_ColorParams));
				g_hash_table_insert(config->bbox_bg_color_table, class_index + (gchar *)NULL, clr_params);
			}
			...
			config->raw_output_directory = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_GIE_RAW_OUTPUT_DIR, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
			config->gpu_id = g_key_file_get_integer(key_file, group, CONFIG_GPU_ID, &error);
...
gboolean parse_tracker(NvDsTrackerConfig *config, GKeyFile *key_file, gchar *cfg_file_path) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			config->enable = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_ENABLE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_WIDTH)) {
			config->width = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_WIDTH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_HEIGHT)) {
			config->height =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_HEIGHT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
			config->gpu_id = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GPU_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_SURFACE_TYPE)) {
			config->tracking_surf_type =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_SURFACE_TYPE, &error);
			CHECK_ERROR(error);
			...
			config->ll_lib_file =
			    get_absolute_file_path(cfg_file_path, g_key_file_get_string(key_file, CONFIG_GROUP_TRACKER,
			                                                                CONFIG_GROUP_TRACKER_LL_LIB_FILE, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_TRACKING_SURFACE_TYPE)) {
			config->tracking_surface_type = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER,
			                                                       CONFIG_GROUP_TRACKER_TRACKING_SURFACE_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_DISPLAY_TRACKING_ID)) {
			config->display_tracking_id = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER,
			                                                     CONFIG_GROUP_TRACKER_DISPLAY_TRACKING_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_TRACKING_ID_RESET_MODE)) {
			config->tracking_id_reset_mode = g_key_file_get_integer(
			    key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_TRACKING_ID_RESET_MODE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_INPUT_TENSOR_META)) {
			config->input_tensor_meta =
			    g_key_file_get_boolean(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_INPUT_TENSOR_META, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_TENSOR_META_GIE_ID)) {
			config->input_tensor_gie_id =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_TENSOR_META_GIE_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_COMPUTE_HW)) {
			config->compute_hw =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_COMPUTE_HW, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_USER_META_POOL_SIZE)) {
			config->user_meta_pool_size = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER,
			                                                     CONFIG_GROUP_TRACKER_USER_META_POOL_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_SUB_BATCHES)) {
			config->sub_batches =
			    g_key_file_get_string(key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_SUB_BATCHES, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_SUB_BATCH_ERR_RECOVERY_TRIAL_CNT)) {
			config->sub_batch_err_recovery_trial_cnt = g_key_file_get_integer(
			    key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_SUB_BATCH_ERR_RECOVERY_TRIAL_CNT, &error);
			CHECK_ERROR(error);
		} else {
			NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, CONFIG_GROUP_TRACKER);
		}
	}
...
gboolean parse_sink(NvDsSinkSubBinConfig *config, GKeyFile *key_file, gchar *group, gchar *cfg_file_path) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			...
			config->type = (NvDsSinkType)g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_ONLY_FOR_DEMUX)) {
			config->link_to_demux = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_ONLY_FOR_DEMUX, &error);
			CHECK_ERROR(error);
			...
			config->sync = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_SYNC, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
			config->render_config.nvbuf_memory_type =
			    g_key_file_get_integer(key_file, group, CONFIG_NVBUF_MEMORY_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_CONTAINER)) {
			config->encoder_config.container =
			    (NvDsContainerType)g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_CONTAINER, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_CODEC)) {
			config->encoder_config.codec =
			    (NvDsEncoderType)g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_CODEC, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_ENC_TYPE)) {
			config->encoder_config.enc_type =
			    (NvDsEncHwSwType)g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_ENC_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_COPY_META)) {
			config->encoder_config.copy_meta =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_COPY_META, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_OUTPUT_IO_MODE)) {
			config->encoder_config.output_io_mode =
			    (NvDsEncOutputIOMode)g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_OUTPUT_IO_MODE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_SW_PRESET)) {
			config->encoder_config.sw_preset =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_SW_PRESET, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_BITRATE)) {
			config->encoder_config.bitrate = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_BITRATE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_PROFILE)) {
			config->encoder_config.profile = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_PROFILE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_IFRAMEINTERVAL)) {
			config->encoder_config.iframeinterval =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_IFRAMEINTERVAL, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_OUTPUT_FILE)) {
			config->encoder_config.output_file_path =
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_OUTPUT_FILE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_SOURCE_ID)) {
			config->source_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_SOURCE_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_RTSP_PORT)) {
			config->encoder_config.rtsp_port =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_RTSP_PORT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_UDP_PORT)) {
			config->encoder_config.udp_port =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_UDP_PORT, &error);
			CHECK_ERROR(error);
			...
			config->render_config.color_range =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_COLOR_RANGE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_CONN_ID)) {
			config->render_config.conn_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_CONN_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_PLANE_ID)) {
			config->render_config.plane_id =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_PLANE_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_SET_MODE)) {
			config->render_config.set_mode =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_SET_MODE, &error);
			CHECK_ERROR(error);
			...
			config->msg_conv_broker_config.proto_lib =
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_PROTO_LIB, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_BROKER_CONN_STR)) {
			config->msg_conv_broker_config.conn_str =
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_CONN_STR, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_BROKER_TOPIC)) {
			config->msg_conv_broker_config.topic =
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_TOPIC, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_BROKER_CONFIG_FILE)) {
			config->msg_conv_broker_config.broker_config_file_path = get_absolute_file_path(
			    cfg_file_path,
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_CONFIG_FILE, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_BROKER_COMP_ID)) {
			config->msg_conv_broker_config.broker_comp_id =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_COMP_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_BROKER_DISABLE_MSG_CONVERTER)) {
			config->msg_conv_broker_config.disable_msgconv =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_DISABLE_MSG_CONVERTER, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_BROKER_NEW_API)) {
			config->msg_conv_broker_config.new_api =
			    g_key_file_get_boolean(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_NEW_API, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_BROKER_SLEEP_TIME)) {
			config->msg_conv_broker_config.broker_sleep_time =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_SLEEP_TIME, &error);
			CHECK_ERROR(error);
		} else {
			NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, group);
		}
...
gboolean parse_tiled_display(NvDsTiledDisplayConfig *config, GKeyFile *key_file) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			config->enable = (NvDsTiledDisplayEnable)g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY,
			                                                                CONFIG_GROUP_ENABLE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_DISPLAY_ROWS)) {
			config->rows =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GROUP_TILED_DISPLAY_ROWS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_DISPLAY_COLUMNS)) {
			config->columns = g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY,
			                                         CONFIG_GROUP_TILED_DISPLAY_COLUMNS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_DISPLAY_WIDTH)) {
			config->width =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GROUP_TILED_DISPLAY_WIDTH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_DISPLAY_HEIGHT)) {
			config->height =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GROUP_TILED_DISPLAY_HEIGHT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
			config->gpu_id = g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GPU_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
			config->nvbuf_memory_type =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_NVBUF_MEMORY_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_COMPUTE_HW)) {
			config->compute_hw =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY, CONFIG_GROUP_TILED_COMPUTE_HW, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_DISPLAY_BUFFER_POOL_SIZE)) {
			config->buffer_pool_size = g_key_file_get_integer(key_file, CONFIG_GROUP_TILED_DISPLAY,
			                                                  CONFIG_GROUP_TILED_DISPLAY_BUFFER_POOL_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TILED_DISPLAY_SQUARE_SEQ_GRID)) {
			config->square_seq_grid = g_key_file_get_boolean(key_file, CONFIG_GROUP_TILED_DISPLAY,
...
gboolean parse_msgconv(NvDsSinkMsgConvBrokerConfig *config, GKeyFile *key_file, gchar *group, gchar *cfg_file_path) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			...
			config->config_file_path = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_CONFIG, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_PAYLOAD_TYPE)) {
			config->conv_payload_type =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_PAYLOAD_TYPE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_MSG2P_LIB)) {
			config->conv_msg2p_lib = get_absolute_file_path(
			    cfg_file_path, g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_MSG2P_LIB, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_COMP_ID)) {
			config->conv_comp_id = g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_COMP_ID, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_DEBUG_PAYLOAD_DIR)) {
			config->debug_payload_dir = get_absolute_file_path(
			    cfg_file_path,
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_DEBUG_PAYLOAD_DIR, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_MULTIPLE_PAYLOADS)) {
			config->multiple_payloads =
			    g_key_file_get_boolean(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_MULTIPLE_PAYLOADS, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_MSG2P_NEW_API)) {
			config->conv_msg2p_new_api =
			    g_key_file_get_boolean(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_MSG2P_NEW_API, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_FRAME_INTERVAL)) {
			config->conv_frame_interval =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_FRAME_INTERVAL, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_DUMMY_PAYLOAD)) {
			config->conv_dummy_payload =
			    g_key_file_get_boolean(key_file, group, CONFIG_GROUP_SINK_MSG_CONV_DUMMY_PAYLOAD, &error);
			CHECK_ERROR(error);
		}
	}
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
			...
			config->output_folder_path =
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_IMG_SAVE_OUTPUT_FOLDER_PATH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_CSV_TIME_RULES_PATH)) {
			config->frame_to_skip_rules_path =
			    g_key_file_get_string(key_file, group, CONFIG_GROUP_IMG_SAVE_CSV_TIME_RULES_PATH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_FULL_FRAME_IMG_SAVE)) {
			config->save_image_full_frame =
			    g_key_file_get_double(key_file, group, CONFIG_GROUP_IMG_SAVE_FULL_FRAME_IMG_SAVE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_CROPPED_OBJECT_IMG_SAVE)) {
			config->save_image_cropped_object =
			    g_key_file_get_double(key_file, group, CONFIG_GROUP_IMG_SAVE_CROPPED_OBJECT_IMG_SAVE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_SECOND_TO_SKIP_INTERVAL)) {
			config->second_to_skip_interval =
			    g_key_file_get_double(key_file, group, CONFIG_GROUP_IMG_SAVE_SECOND_TO_SKIP_INTERVAL, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_MIN_CONFIDENCE)) {
			config->min_confidence =
			    g_key_file_get_double(key_file, group, CONFIG_GROUP_IMG_SAVE_MIN_CONFIDENCE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_MAX_CONFIDENCE)) {
			config->max_confidence =
			    g_key_file_get_double(key_file, group, CONFIG_GROUP_IMG_SAVE_MAX_CONFIDENCE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_MIN_BOX_WIDTH)) {
			config->min_box_width =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_IMG_SAVE_MIN_BOX_WIDTH, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_IMG_SAVE_MIN_BOX_HEIGHT)) {
			config->min_box_height =
			    g_key_file_get_integer(key_file, group, CONFIG_GROUP_IMG_SAVE_MIN_BOX_HEIGHT, &error);
			CHECK_ERROR(error);
		} else {
			NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, group);
		}
