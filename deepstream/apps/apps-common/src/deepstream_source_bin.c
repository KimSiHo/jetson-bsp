...
static gboolean create_camera_source_bin(NvDsSourceConfig *config, NvDsSrcBin *bin) {
	...
	if (config->video_format) {
		caps = gst_caps_new_simple("video/x-raw", "format", G_TYPE_STRING, config->video_format, "width", G_TYPE_INT,
		                           config->source_width, "height", G_TYPE_INT, config->source_height, "framerate",
		                           GST_TYPE_FRACTION, config->source_fps_n, config->source_fps_d, NULL);
	} else {
...
static gboolean seek_decode(gpointer data) {
	...
	ret = gst_element_seek(bin->bin, 1.0, GST_FORMAT_TIME, (GstSeekFlags)(GST_SEEK_FLAG_KEY_UNIT | GST_SEEK_FLAG_FLUSH),
...
static void decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
	...
	if (g_strstr_len(name, -1, "nvv4l2decoder") == name) {
		...
		if (config->loop && g_strstr_len(config->uri, -1, "file:/") == config->uri) {
			NVGSTDS_ELEM_ADD_PROBE(bin->src_buffer_probe, GST_ELEMENT(object), "sink", restart_stream_buf_prob,
...
static void check_rtsp_reconnection_attempts(NvDsSrcBin *src_bin) {
	...
	if (remove_probe) {
		...
		NVGSTDS_ELEM_REMOVE_PROBE(src_bin->parent_bin->nvstreammux_eosmonitor_probe, src_bin->parent_bin->streammux,
		                          "src");
...
static gboolean create_rtsp_src_bin(NvDsSourceConfig *config, NvDsSrcBin *bin) {
	...
	if ((config->select_rtp_protocol == GST_RTSP_LOWER_TRANS_TCP) ||
		...
		GST_DEBUG_OBJECT(bin->src_elem, "RTP Protocol=0x%x (0x4=TCP and 0x7=UDP,TCP,UDPMCAST)----\n",
		                 config->select_rtp_protocol);
	}
	...
	if (bin->rtsp_reconnect_interval_sec > 0) {
		NVGSTDS_ELEM_ADD_PROBE(bin->rtspsrc_monitor_probe, bin->dec_que, "sink", rtspsrc_monitor_probe_func,
	...
	g_object_set(G_OBJECT(bin->nvvidconv), "gpu-id", config->gpu_id, "nvbuf-memory-type", config->nvbuf_memory_type,
	             NULL);
	...
	gst_caps_unref(caps);
	gst_bin_add_many(GST_BIN(bin->bin), bin->src_elem, bin->tee_rtsp_pre_decode, bin->dec_que, bin->decodebin,
	                 bin->cap_filter, bin->tee_rtsp_post_decode, bin->nvvidconv, bin->cap_filter1, NULL);
...
static gboolean create_audiodecode_src_bin(NvDsSourceConfig *config, NvDsSrcBin *bin) {
	...
	if (config->type == NV_DS_SOURCE_AUDIO_WAV) {
		gst_bin_add_many(GST_BIN(bin->bin), bin->src_elem, bin->decodebin, bin->audio_converter, bin->audio_resample,
		                 bin->cap_filter, NULL);

		gst_element_link_many(bin->src_elem, bin->decodebin, bin->audio_converter, bin->audio_resample, bin->cap_filter,
		                      NULL);
	} else if (config->type == NV_DS_SOURCE_ALSA_SRC) {
		gst_bin_add_many(GST_BIN(bin->bin), bin->src_elem, bin->audio_converter, bin->audio_resample, bin->cap_filter,
		                 NULL);
...
#if defined(__aarch64__) && !defined(AARCH64_IS_SBSA)
	/* For Jetson, with copy-hw=1 and memory-type=nvbuf-mem-surface-array,
	         cudaMemcopy fail is observed. This is a WAR till root cause is
	   fixed
	 */
	g_object_set(G_OBJECT(bin->nvvidconv), "copy-hw", 2, NULL);
#endif
...
gboolean create_multi_source_bin(guint num_sub_bins, NvDsSourceConfig *configs, NvDsSrcParentBin *bin) {
	...
	if (install_mux_eosmonitor_probe) {
		NVGSTDS_ELEM_ADD_PROBE(bin->nvstreammux_eosmonitor_probe, bin->streammux, "src",
		                       nvstreammux_eosmonitor_probe_func, GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM, bin);
	}
