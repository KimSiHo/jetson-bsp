...
static void all_bbox_generated(AppCtx *appCtx, GstBuffer *buf, NvDsBatchMeta *batch_meta, guint index) {
	...
	for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
		...
		packet.frame_num = frame_meta->frame_num;
		packet.object_count = 0;
		frame_count++;
		...
		for (NvDsMetaList *l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
			...
			if (packet.object_count >= MAX_OBJECTS) {
				NVGSTDS_ERR_MSG_V("exceed MAX_OBJECTS");
				continue;
			}
			DetectionMsg *msg = &packet.objects[packet.object_count++];

			msg->object_id = obj->object_id;
			msg->class_id = obj->class_id;
			strncpy(msg->label, obj->obj_label, MAX_LABEL_LEN - 1);
			msg->label[MAX_LABEL_LEN - 1] = '\0';
			msg->confidence = obj->confidence;
			msg->tracker_confidence = obj->tracker_confidence;

			msg->left = obj->rect_params.left;
			msg->top = obj->rect_params.top;
			msg->width = obj->rect_params.width;
			msg->height = obj->rect_params.height;
		}

		if (uds_client_send_packet(&packet)) {
			NVGSTDS_ERR_MSG_V("Failed to send UDS packet");
		}
	}
}
...
static void print_runtime_commands(void) {
	...
	if (appCtx[0]->config.tiled_display_config.enable) {
		g_print("NOTE: To expand a source in the 2D tiled display and view object "
		        "details,"
...
static void smart_record_start(AppCtx *appCtx, guint source_id) {
	...
	if (!ctx || src_bin->reconfiguring) {
		return;
	}

	g_print("recordOn = %s\n", ctx->recordOn ? "TRUE" : "FALSE");
	if (!ctx->recordOn) {
		guint startTime = src_bin->config->smart_rec_start_time;
		guint duration = src_bin->config->smart_rec_duration;

		NvDsSRStart(ctx, &sessId, startTime, duration, NULL);
		g_print("Smart record START\n");
	}
}

static void smart_record_stop(AppCtx *appCtx, guint source_id) {
	NvDsSrcBin *src_bin = &appCtx->pipeline.multi_src_bin.sub_bins[source_id];
	NvDsSRContext *ctx = (NvDsSRContext *)src_bin->recordCtx;
	...
	if (ctx->recordOn) {
		NvDsSRStop(ctx, 0);
		g_print("Smart record STOP\n");
	}
}

static gboolean uds_poll(gpointer data) {
	AppCtx *appCtx = (AppCtx *)data;
	char buf[64];

	if (uds_client_recv(buf, sizeof(buf)) > 0) {
		g_print("recv: %s\n", buf);

		if (strcmp(buf, "START_RECORDING") == 0)
			smart_record_start(appCtx, 0);
		else if (strcmp(buf, "STOP_RECORDING") == 0)
			smart_record_stop(appCtx, 0);
	}
...
int main(int argc, char *argv[]) {
	...
	ctx = g_option_context_new("Nvidia DeepStream Proximity");
	...
	for (i = 0; i < num_instances; i++) {
		...
		if (g_str_has_suffix(cfg_files[i], ".txt")) {
	...
	if (uds_client_init(DS_SOCKET_PATH)) {
		NVGSTDS_ERR_MSG_V("Failed to connect UDS server: %s", DS_SOCKET_PATH);
		return -1;
	}
	...
	for (i = 0; i < num_instances; i++) {
		...
		GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN(appCtx[i]->pipeline.pipeline), GST_DEBUG_GRAPH_SHOW_ALL,
		                                  "deepstream_pipeline");
	}
	...
	g_timeout_add(40, uds_poll, appCtx[0]);
	...
	if (return_value != -1) {
		for (i = 0; i < num_instances; i++) {
			...
			if (appCtx[i]->config.pipeline_recreate_sec) {
				g_timeout_add_seconds(appCtx[i]->config.pipeline_recreate_sec, recreate_pipeline_thread_func,
				                      appCtx[i]);
			}
		}
	}
