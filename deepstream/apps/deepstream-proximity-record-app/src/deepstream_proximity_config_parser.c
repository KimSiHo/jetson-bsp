...
static gboolean parse_source_list(NvDsConfig *config, GKeyFile *key_file, gchar *cfg_file_path) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_NUM_SOURCE_BINS)) {
			config->total_num_sources = g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST,
			...
			config->sensor_id_list =
			    g_key_file_get_string_list(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_SENSOR_ID_LIST,
			...
			config->use_nvmultiurisrcbin = g_key_file_get_boolean(
			    key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_USE_NVMULTIURISRCBIN, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_STREAM_NAME_DISPLAY)) {
			config->stream_name_display = g_key_file_get_boolean(key_file, CONFIG_GROUP_SOURCE_LIST,
			                                                     CONFIG_GROUP_SOURCE_LIST_STREAM_NAME_DISPLAY, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_HTTP_IP)) {
			config->http_ip =
			    g_key_file_get_string(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_HTTP_IP, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_HTTP_PORT)) {
			config->http_port =
			    g_key_file_get_string(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_HTTP_PORT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_MAX_BATCH_SIZE)) {
			config->max_batch_size = g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST,
			...
			config->sgie_batch_size =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_SGIE_BATCH_SIZE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_EXTRACT_SEI_TYPE5_DATA)) {
			config->extract_sei_type5_data = g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST,
			                                                        CONFIG_GROUP_SOURCE_EXTRACT_SEI_TYPE5_DATA, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LIST_LOW_LATENCY_MODE)) {
			config->low_latency_mode = g_key_file_get_integer(key_file, CONFIG_GROUP_SOURCE_LIST,
			                                                  CONFIG_GROUP_SOURCE_LIST_LOW_LATENCY_MODE, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_SEI_UUID)) {
			config->sei_uuid =
			    g_key_file_get_string(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_SEI_UUID, &error);
			CHECK_ERROR(error);
		} else {
	...
	if (g_key_file_has_key(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_URI_LIST, &error)) {
		if (g_key_file_has_key(key_file, CONFIG_GROUP_SOURCE_LIST, CONFIG_GROUP_SOURCE_LIST_NUM_SOURCE_BINS, &error)) {
...
static gboolean set_source_all_configs(NvDsConfig *config, gchar *cfg_file_path) {
	...
	for (i = 0; i < config->total_num_sources; i++) {
		...
		if (config->uri_list) {
			...
			if (!uri) {
				NVGSTDS_ERR_MSG_V("uri %d entry of list is NULL, use valid uri "
				                  "separated by ';' with the source-list section",
				                  (i + 1));
				return FALSE;
			}
			if (g_str_has_prefix(config->uri_list[i], "file://")) {
				...
				config->multi_source_config[i].uri = g_strdup_printf(
				    "file://%s", get_absolute_file_path(cfg_file_path, config->multi_source_config[i].uri));
				...
				if (source_id_start_ptr == source_id_end_ptr || *source_id_end_ptr != '\0') {
					NVGSTDS_ERR_MSG_V("Incorrect URI for camera source %s. FORMAT: "
...
static gboolean parse_tests(NvDsConfig *config, GKeyFile *key_file) {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_TESTS_FILE_LOOP)) {
			config->file_loop =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TESTS, CONFIG_GROUP_TESTS_FILE_LOOP, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_TESTS_PIPELINE_RECREATE_SEC)) {
			config->pipeline_recreate_sec =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_TESTS, CONFIG_GROUP_TESTS_PIPELINE_RECREATE_SEC, &error);
			CHECK_ERROR(error);
		} else {
	...
	for (key = keys; *key; key++) {
		if (!g_strcmp0(*key, CONFIG_GROUP_APP_ENABLE_PERF_MEASUREMENT)) {
			config->enable_perf_measurement =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_ENABLE_PERF_MEASUREMENT, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_APP_PERF_MEASUREMENT_INTERVAL)) {
			config->perf_measurement_interval_sec =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_PERF_MEASUREMENT_INTERVAL, &error);
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_APP_GIE_OUTPUT_DIR)) {
			config->bbox_dir_path =
			    get_absolute_file_path(cfg_file_path, g_key_file_get_string(key_file, CONFIG_GROUP_APP,
			...
			config->kitti_track_dir_path = get_absolute_file_path(
			    cfg_file_path,
			    g_key_file_get_string(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_GIE_TRACK_OUTPUT_DIR, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_APP_REID_TRACK_OUTPUT_DIR)) {
			config->reid_track_dir_path = get_absolute_file_path(
			    cfg_file_path,
			    g_key_file_get_string(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_REID_TRACK_OUTPUT_DIR, &error));
			CHECK_ERROR(error);
		} else if (!g_strcmp0(*key, CONFIG_GROUP_APP_GLOBAL_GPU_ID)) {
			/** App Level GPU ID is set here if it is present in APP LEVEL
			 * config group if gpu_id prop is not set for any component, this
			 * global_gpu_id will be used */
			config->global_gpu_id =
			    g_key_file_get_integer(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_GLOBAL_GPU_ID, &error);
			CHECK_ERROR(error);
			...
			config->shadow_track_output_path = get_absolute_file_path(
			    cfg_file_path,
			    g_key_file_get_string(key_file, CONFIG_GROUP_APP, CONFIG_GROUP_APP_SHADOW_TRACK_OUTPUT_DIR, &error));
			CHECK_ERROR(error);
		} else {
...
gboolean parse_config_file(NvDsConfig *config, gchar *cfg_file_path) {
	...
	if (g_key_file_has_group(cfg_file, CONFIG_GROUP_SOURCE_ALL)) {
		 ...
		 * it will override the value set using global_gpu_id in parse_source
		 * function */
		if (!parse_source(&config->source_attr_all_config, cfg_file, (gchar *)CONFIG_GROUP_SOURCE_ALL, cfg_file_path)) {
			GST_CAT_ERROR(APP_CFG_PARSER_CAT, "Failed to parse '%s' group", CONFIG_GROUP_SOURCE_LIST);
			goto done;
		}
	...
	for (group = groups; *group; group++) {
		...
		if (!strncmp(*group, CONFIG_GROUP_SOURCE, sizeof(CONFIG_GROUP_SOURCE) - 1)) {
			...
			if (source_id_start_ptr == source_id_end_ptr || *source_id_end_ptr != '\0') {
				NVGSTDS_ERR_MSG_V("Source group \"[%s]\" is not in the form "
				                  "\"[source<%%d>]\"",
				                  *group);
				ret = FALSE;
				goto done;
			...
			/** if gpu_id for source component is present,
			 * it will override the value set using global_gpu_id in
			 * parse_source function */
			parse_err = !parse_source(&config->multi_source_config[source_id], cfg_file, *group, cfg_file_path);
			if (config->source_list_enabled &&
			    config->multi_source_config[source_id].type == NV_DS_SOURCE_URI_MULTIPLE) {
				NVGSTDS_ERR_MSG_V("MultiURI support not available if "
				                  "[source-list] is provided");
				ret = FALSE;
				goto done;
			}
		...
		if (!g_strcmp0(*group, CONFIG_GROUP_OSD)) {
			 ...
			 * it will override the value set using global_gpu_id in parse_osd
			 * function */
		...
		if (!g_strcmp0(*group, CONFIG_GROUP_PRIMARY_GIE)) {
			 ...
			 * it will override the value set using global_gpu_id in parse_gie
			 * function */
			parse_err =
			    !parse_gie(&config->primary_gie_config, cfg_file, (gchar *)CONFIG_GROUP_PRIMARY_GIE, cfg_file_path);
		}
		...
		if (!strncmp(*group, CONFIG_GROUP_SECONDARY_GIE, sizeof(CONFIG_GROUP_SECONDARY_GIE) - 1)) {
			 ...
			 * it will override the value set using global_gpu_id in parse_gie
			 * function */
			parse_err = !parse_gie(&config->secondary_gie_sub_bin_config[config->num_secondary_gie_sub_bins], cfg_file,
			                       *group, cfg_file_path);
		...
		if (!strncmp(*group, CONFIG_GROUP_SECONDARY_PREPROCESS, sizeof(CONFIG_GROUP_SECONDARY_PREPROCESS) - 1)) {
			if (config->num_secondary_preprocess_sub_bins == MAX_SECONDARY_PREPROCESS_BINS) {
				NVGSTDS_ERR_MSG_V("App supports max %d secondary PREPROCESSs", MAX_SECONDARY_PREPROCESS_BINS);
				ret = FALSE;
				goto done;
			}
			parse_err = !parse_preprocess(
			    &config->secondary_preprocess_sub_bin_config[config->num_secondary_preprocess_sub_bins], cfg_file,
			    *group, cfg_file_path);
		...
		if (!strncmp(*group, CONFIG_GROUP_SINK, sizeof(CONFIG_GROUP_SINK) - 1)) {
			...
			if (config->global_gpu_id != -1) {
				GError *error = NULL;
				if (g_key_file_get_integer(cfg_file, *group, "enable", &error) == TRUE && error == NULL) {
					config->sink_bin_sub_bin_config[config->num_sink_sub_bins].encoder_config.gpu_id =
					    config->sink_bin_sub_bin_config[config->num_sink_sub_bins].render_config.gpu_id =
					        config->global_gpu_id;
				}
			}
			/** if gpu_id for sink component is present,
			 * it will override the value set using global_gpu_id in parse_sink function */
			parse_err = !parse_sink(&config->sink_bin_sub_bin_config[config->num_sink_sub_bins], cfg_file, *group,
			                        cfg_file_path);
	...
	for (i = 0; i < config->num_source_sub_bins; i++) {
		if (config->multi_source_config[i].type == NV_DS_SOURCE_URI_MULTIPLE) {
			...
			for (j = 1; j < config->multi_source_config[i].num_sources; j++) {
				...
				memcpy(&config->multi_source_config[config->num_source_sub_bins], &config->multi_source_config[i],
